...
int main(int argc, char *argv[]) {
    ...
    cout << "enter m for (m,n) predictor: ";
    int m = 2;  // set 2 for default
    cin >> m;
    int column = (int)pow(2, m), row = (int)pow(2, 10 - m);
    int predictor_stat[column][row] = {3};
    for (int i = 1; i <= 10; i++) {
        ...
        while (!testbench.eof()) {
            ...
            int status = predictor_stat[branch_history][addr % row];
            ...
            if (status == 3) {
                ...
                if (t_or_n == 'T') {
                    ...
                    predictor_stat[branch_history][addr % row] = 2;
                }
            } else if (status == 2) {
                prediction = 'T';
                if (t_or_n == 'T') {
                    correct_predict++;
                    predictor_stat[branch_history][addr % row] = 3;
                } else {
                    predictor_stat[branch_history][addr % row] = 0;
                }
            } else if (status == 0) {
                prediction = 'N';
                if (t_or_n == 'N') {
                    ...
                    predictor_stat[branch_history][addr % row] = 1;
                }
            } else {
                prediction = 'N';
                if (t_or_n == 'N') {
            ...
            branch_history = (branch_history * 2 + ((t_or_n == 'T') ? 1 : 0)) % column;
